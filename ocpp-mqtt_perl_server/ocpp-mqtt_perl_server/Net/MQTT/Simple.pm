...
#
# SPDX-FileCopyrightText: 2024 Juerd Waalboer <juerd@tnx.nl> - https://metacpan.org/pod/Net::MQTT::Simple
#
...
sub subscribe {
    ...
    while (my ($topic, $callback) = splice @kv, 0, 2) {
        ...
        # Be sure no other callbacks with the same topic are in place:
        my $callbackset=0;
        foreach my $cb (@{ $self->{callbacks} }) {
            if($cb->{topic} eq $topic) {
